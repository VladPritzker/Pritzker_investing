...
const StockDataPdfModal = ({ isOpen, onClose }) => {
  ...
  const fetchStockData = async () => {
    try {
      const response = await fetch(`${apiUrl}/fetch-stock-data/?symbols=TSLA,AMZN,MSFT`);
      if (response.ok) {
  ...
  return (
    <div className="stock-data-modal" onClick={handleBackdropClick}>
      <div className="stock-data-modal-content">
        ...
        <div>
          <button className="stock-data-modal-button" onClick={downloadPdf}>Download PDF</button>
