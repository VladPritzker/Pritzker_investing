...
const ChartModal = ({ sleepLogs, onClose }) => {
    ...
    const filterLogsByPeriod = (logs) => {
        if (startDate && endDate) {
            ...
            return logs.filter(log => new Date(log.date) >= start && new Date(log.date) <= end);
        }
        return logs;
    };
    ...
    const data = {
        ...
        datasets: [
            {
                label: 'Sleep Time',
                data: filteredLogs.map(log => new Date(log.sleep_time).getHours() + new Date(log.sleep_time).getMinutes() / 60),
                fill: false,
            ...
            {
                label: 'Wake Time',
                data: filteredLogs.map(log => new Date(log.wake_time).getHours() + new Date(log.wake_time).getMinutes() / 60),
                fill: false,
