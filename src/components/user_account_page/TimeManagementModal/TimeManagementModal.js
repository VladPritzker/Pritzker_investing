...
const SleepLogsModal = ({ userId, sleepLogs, setSleepLogs, onClose }) => {
  ...
  const handleSave = async () => {
    try {
      const response = await fetch(
        `${apiUrl}/sleeplogs/${userId}/${editLogId}/`,
        {
          method: "PATCH",
          headers: {
  ...
  const confirmDelete = async () => {
    try {
      const response = await fetch(
        `${apiUrl}/sleeplogs/${userId}/${deleteLogId}/`,
        {
          method: "DELETE",
        },
      );
      if (response.ok) {
        setSleepLogs((prevLogs) =>
          prevLogs.filter((log) => log.id !== deleteLogId),
        );
    ...
    <div className="modal-overlay">
      <div className="sleep-logs-modal">
        ...
        <div className="button-container">
          ...
          <button onClick={handleOpenChartModal} className="chart-button">
            Chart
          </button>
        </div>
              ...
              <div key={log.id} className="sleep-log-entry">
                {editLogId === log.id ? (
                  ...
                  <>
                    ...
                    <p>
                      <strong>Sleep Time:</strong> {formatTime(log.sleep_time)}
                    </p>
                    <p>
                      <strong>Wake Time:</strong> {formatTime(log.wake_time)}
                    </p>
                    <div className="addLogButtons">
                      <button
      ...
      {isChartModalOpen && (
        <ChartModal sleepLogs={sleepLogs} onClose={handleCloseChartModal} />
      )}
      {selectedLog && (
        <SleepLogPopup log={selectedLog} onClose={() => setSelectedLog(null)} />
      )}
    </div>
  );
};
